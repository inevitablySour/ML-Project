...
cols_to_drop = [
    ...
    'Date',            # Already have race_year, race_month, race_day_of_week
    ...
    'Pnt_filled',      # Current race points - 99.1% of top-10 have points (LEAKAGE!)
...
leakage_cols = ['Pnt_filled', 'Pnt_total', 'Pnt_max', 'Pnt_std']
...
print(f"   Dropped other columns: {[c for c in cols_to_drop if c not in leakage_found]}")
print(f"   Remaining columns: {df.shape[1]}")
print(f"\n   Kept historical Pnt features (no leakage):")
historical_pnt = [c for c in df.columns if 'Pnt' in c or 'pnt' in c]
if historical_pnt:
...
if medium_cardinality_cols:
    ...
    for col in medium_cardinality_cols:
        ...
        encoding_map = df.groupby(col)['is_top_10'].mean().to_dict()
...
# ============================================================================
# 5. TRAIN/TEST SPLIT (BEFORE SMOTE to avoid data leakage)
# ============================================================================
print("\n[5/7] Splitting into train/test sets...")
